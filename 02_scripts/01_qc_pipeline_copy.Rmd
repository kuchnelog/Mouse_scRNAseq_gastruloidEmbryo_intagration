...
#     DATASET:
#         value: NA
#         choices: [fibroWT, fibroKO, cardioWT, cardioKO]
#     PATH_ROOT: NA
#     PATH_INPUT_LABDATA: NA
#     PATH_RDS_OBJECTS: NA
#     PATH_OUT_FIG: NA
#     mito_high:
#         value: 10
...
```{r load-data}
raw_data <- Read10X(data.dir = PATH_INPUT_LABDATA, gene = 2)
...
SO <- AddMetaData(object = SO, metadata = type,    col.name = "type")
...
cellnames <- paste(type, UMIs, sep = "_")
...
SO@meta.data[SO@meta.data$percent.mito > mito_high, "outlier.mito"] <- TRUE
SO@meta.data[SO@meta.data$percent.mito < mito_high, "outlier.mito"] <- FALSE

plot1 <- VlnPlot(SO, features = c("percent.mito")) +
    geom_hline(aes(yintercept = mito_high), color = "#FF0000AA", size = 1.5) +
    ggtitle("Mitochondrial expression\npercentage", subtitle = paste0("threshold at ", mito_high, "%")) +
...
plot2 <- VlnPlot(SO, features = c("percent.mito")) +
    geom_hline(aes(yintercept = mito_high), color = "#FF0000AA", size = 1.5) +
    ggtitle("Zoom in", subtitle = paste0("threshold at ", mito_high, "%")) +
    theme(axis.title.x = element_blank(),
          axis.text = element_text(size = 16),
          axis.text.x = element_text(angle = 0, size = 24, hjust = 0.5, face = "bold"),
          text = element_text(size = 24),
...
write.table(
    ...
    file = file.path(
        PATH_OUT_ANALYSIS, 
        paste0("cellsList_filteredOut_", DATASET, "_mito.", mito_high, ".txt")), 
    row.names = FALSE, 
    col.names = FALSE
)
```

We will remove `r table(SO$outlier.mito)["TRUE"]` cells identified having a high
mitochondrial gene expression.  
The threshold upon the mitochondrial gene expression is set at `r mito_high`%.
...
VlnPlot(SO, features = c("percent.ribo")) +
    geom_hline(aes(yintercept = ribo_low), color = "#FF0000AA", size = 1.5) +
    ggtitle("Ribosomal expression\npercentage", subtitle = paste0("threshold at ", ribo_low, "%")) +
    theme(axis.title.x = element_blank(),
          axis.text = element_text(size = 16),
          axis.text.x = element_text(angle = 10, size = 24, hjust = 0.5, face = "bold"),
          text = element_text(size = 24),
          plot.subtitle = element_text(hjust = 0.5)) +
    NoLegend() +
    scale_y_continuous(labels = scales::percent_format(scale = 1))
...
plot1 <- VlnPlot(SO, features = c("nCount_RNA")) +
    geom_hline(aes(yintercept = max_counts), color = "#FF0000AA", size = 1.5) +
    geom_hline(aes(yintercept = min_counts), color = "#FF0000AA", size = 1.5) +
    ggtitle("UMI counts per cell", subtitle = paste0("thresholds at ", min_counts, " and ", max_counts)) +
    ylab("nCount_RNA") +
    theme(axis.title.x = element_blank(), 
          axis.text = element_text(size = 16), 
          axis.text.x = element_text(angle = 0, size = 24, hjust = 0.5, face = "bold"), 
...
VlnPlot(SO, features = c("nFeature_RNA")) +
    geom_hline(aes(yintercept = min_feat), color = "#FF0000AA", size = 1.5) +
    ggtitle("Number of detected features\nper cell", subtitle = paste0("threshold at ", min_feat, " counts")) +
    ylab("nFeature_RNA") +
    theme(axis.title.x = element_blank(),
          axis.text = element_text(size = 16),
          axis.text.x = element_text(angle = 0, size = 24, hjust = 0.5, face = "bold"),
...
```
The number of cells removed by the application of all the filters is `r dim(SO@meta.data[SO@meta.data$percent.mito > mito_high | SO@meta.data$percent.ribo < ribo_low | SO@meta.data$nCount_RNA < min_counts | SO@meta.data$nCount_RNA > max_counts | SO@meta.data$nFeature_RNA < min_feat, ])[1]`.
...
We export metadata for the filtered and complete dataset. We then reduce the
metadata table to the default metadata and save the filtered and complete
objects.
...
write.table(
    ...
    file.path(
        PATH_OUT_ANALYSIS, 
        paste0("metadata_", DATASET, "_complete.csv")),
    row.names = FALSE,
    col.names = TRUE,
    sep = ","
)
...
SO@meta.data <- SO@meta.data[, keepMDnames]
saveRDS(
    SO,
    file.path(PATH_RDS_OBJECTS, paste0("01_qc_", DATASET, "_complete.rds")
    )
)


# add rownames in the first column & get names of MD to export
SO_subset@meta.data <- cbind(cellIDs = rownames(SO_subset@meta.data), SO_subset@meta.data)
exportMD <- names(SO_subset@meta.data)[! names(SO_subset@meta.data) %in% keepMDnames]

# export MD
write.table(
    SO_subset@meta.data[exportMD],
    file.path(
        PATH_OUT_ANALYSIS, 
        paste0("metadata_", DATASET, "_filtered.csv")),
    row.names = FALSE,
    col.names = TRUE,
    sep = ","
)

# reduce MD to those by default & save the object
SO_subset@meta.data <- SO_subset@meta.data[, keepMDnames]
saveRDS(
    SO_subset,
    file.path(PATH_RDS_OBJECTS, paste0("01_qc_", DATASET, "_filtered.rds")
    )
)
```
