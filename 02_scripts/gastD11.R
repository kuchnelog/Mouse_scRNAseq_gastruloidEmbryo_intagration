...
# 4. Normalization -------------------------------------------------------------
gs <- NormalizeData(gs, normalization.method = norm_method, scale.factor = 10000)

# 5. Identification of highly variable features (feature selection) ------------
gs <- FindVariableFeatures(gs, selection.method = var_feat_method, nfeatures = 2000)
...
gs <- ScaleData(gs, features = all.genes)
...
gs <- RunUMAP(gs, dims = 1:20, seed.use = general.seed)
...
gs.markers <- FindAllMarkers(gs, group.by = "RNA_snn_res.1", only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
gs.markers %>%
  group_by(cluster) %>%
...
gs.sing <- FindClusters(gs.sing, resolution = res_var, algorithm = Leiden, random.seed = general.seed) # random.seed = general.seed, verbose=FALSE
## Run non-linear dimensional reduction (UMAP/tSNE) ----------------------------
gs.sing <- RunUMAP(gs.sing, dims = 1:20, seed.use = general.seed)
...
gs.sing.markers <- FindAllMarkers(gs.sing, group.by = "RNA_snn_res.1" , only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
